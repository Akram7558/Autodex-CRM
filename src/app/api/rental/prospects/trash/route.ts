// ─────────────────────────────────────────────────────────────────────
// POST /api/rental/prospects/trash
// ─────────────────────────────────────────────────────────────────────
// Soft-delete ("corbeille", migration 51) a batch of CANCELLED prospects.
// The cancelled prospect status is 'perdue' (displayed as "Annulée").
// Body: { ids: string[] }.
//
...
//     has status='perdue'. Ids not matching (wrong status, other showroom,
//     unknown) are silently skipped — never an error.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // RLS-aware update via authSb. Only 'perdue', not-already-trashed rows
